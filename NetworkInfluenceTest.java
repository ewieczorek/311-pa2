...
public class NetworkInfluenceTest {
    NetworkInfluence ni = new NetworkInfluence("C:\\Users\\ethantw\\Dropbox (Film Class)\\CprE 311\\pa2\\inftest.txt");;
    ...
    public void testDistanceStringString() {
        ...
        int d2 = ni.distance("A", "B");
        Assert.assertEquals(d2,1);
        int d3 = ni.distance("A", "C");
        Assert.assertEquals(d3,1);
        int d4 = ni.distance("A", "D");
        Assert.assertEquals(d4,1);
        int d5 = ni.distance("A", "E");
        Assert.assertEquals(d5,2);
        int d6 = ni.distance("A", "F");
        Assert.assertEquals(d6,2);
        int d7 = ni.distance("A", "G");
        Assert.assertEquals(d7,2);
        int d8 = ni.distance("A", "H");
        Assert.assertEquals(d8,2);
        int d9 = ni.distance("X", "Y");//no path
        Assert.assertEquals(d9,-1);
        int d10 = ni.distance("D", "E");//no path
        Assert.assertEquals(d10,-1);
    }
    ...
    public void testMostInfluentialDegree() {
        ...
        Assert.assertEquals(most,new ArrayList<String>(Arrays.asList(new String[] {"B","A"})));
